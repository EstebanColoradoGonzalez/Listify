...
object Queries {
    ...
    const val SELECT_SHOPPING_LIST_DTO_BY_ID = "SELECT s.id as id, s.shopping_list_date as date, st.name as status FROM shopping_list s JOIN shopping_list_state ss ON s.id = ss.shopping_list JOIN state st ON ss.state = st.id WHERE s.id = :shoppingList"
